let changeBackgroundImage = (clickedImage) => {
  ...
  images.forEach((image) =>
    image.parentElement.classList.remove("active-image")
  );
...
let changeBackgroundImageSeason = (clickedImage) => {
  ...
  const images = document.querySelectorAll(".images img");
  images.forEach((image) =>
    image.parentElement.classList.remove("seasons-active-image")
  );
...
let changeBackgroundImageSeries = (clickedImage) => {
  ...
  const images = document.querySelectorAll(".images img");
  images.forEach((image) =>
    image.parentElement.classList.remove("tv-series-active-image")
  );
